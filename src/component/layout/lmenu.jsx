...
export class Lmenu extends Component {
	...
	render() {
		const defaultSelectedKey = process.env.NODE_ENV !== 'production' ? [location.pathname.split('/')[location.pathname.split('/').length - 1] || 'zhibanbiao'] : [location.hash.split('/')[location.hash.split('/').length - 1].split('?')[0] || 'leifengjiao'];
		return (
